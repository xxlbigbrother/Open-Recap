...
def _build_visual_qc(tts_segments, work_dir, video_duration, canvas, *, overlay_qc=None, mask_filter=None):
    ...
    mask.update({
        ...
        "bridge_seconds": CONFIG.get("subtitle_mask_bridge_seconds", 0.001),
...
def _subtitle_burn_filter(subtitle_path):
    ...
    import os
    suffix = ""
    if os.environ.get("SUBTITLE_FONTS_DIR"):
        suffix = f":fontsdir='{_escape_subtitle_filter_path(os.environ['SUBTITLE_FONTS_DIR'])}'"
    return f"subtitles=filename='{_escape_subtitle_filter_path(subtitle_path)}'" + suffix
...
def _source_subtitle_mask_filter(canvas, work_dir, tts_segments, video_duration):
    ...
    if opacity > 0:
        if timing == "all":
            ...
            extra_path=Path(work_dir)/'source_subtitle_mask_windows.json'
            if extra_path.exists():
                extra=json.loads(extra_path.read_text())
                if not isinstance(extra,dict) or extra.get('schema_version')!=1 or extra.get('clock')!='output' or not isinstance(extra.get('windows'),list):
                    raise ValueError('invalid source subtitle mask windows schema')
                for row in extra['windows']:
                    if not isinstance(row,dict):raise ValueError('invalid source subtitle mask window')
                    a,b=row.get('start'),row.get('end')
                    if not all(isinstance(x,(int,float)) and not isinstance(x,bool) and math.isfinite(x) for x in (a,b)) or not 0<=a<b<=video_duration+.01:
                        raise ValueError('invalid source subtitle mask window range')
                    windows.append((a,min(b,video_duration),0.0))
            ...
            filters.extend(
                ...
                for start, end, _ in coalesce_duck_windows(windows, bridge=CONFIG.get("subtitle_mask_bridge_seconds", 0.001))
            )
