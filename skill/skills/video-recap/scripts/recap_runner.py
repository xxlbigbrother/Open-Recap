...
from recap_runtime import (
    ...
    _run_film_research_if_requested,
    _run,
...
def main():
    ...
    if args.tts_provider in {"fish-audio", "doubao-tts"} and (explicit_mimo_voice or args.voice_ref):
        ap.error(
    ...
    if args.edit_mode == "dub" and args.tts_provider in {"fish-audio", "doubao-tts"}:
        ap.error(
    ...
    def _understand():
        ...
        if _run_film_research_if_requested(work_dir, args):
            # Research belongs after ASR/VLM. Rebuild only the writing brief so
            # film_research.json is visible without invalidating understanding.
    ...
    if not cut:
        ...
        if not narration_json.exists():
            ...
            return
        # Cut QC refresh can change the validated-plan fingerprint even on a cache hit.
        # Recompute output speech evidence from current source anchors before validation.
        _rebuild_output_brief()
        if _cut_narration_is_stale(_read_phase_ledger(work_dir), cp_fp):
            raise SystemExit(
                "clip_plan.json 已改变，但 narration.json 仍是对旧剪辑写的，会与剪后画面对不上。"
