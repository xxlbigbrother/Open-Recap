...
SUPPORTED_TTS_ENGINES = {"mimo-tts", "fish-audio", "doubao-tts"}
...
def _synthesize_segment(i, seg, narration, tts_dir, engine):
    ...
    if engine == "doubao-tts" and dur > raw_budget:
        raise RuntimeError(f"Narration exceeds safe speech budget ({dur:.2f}s > {raw_budget:.2f}s); shorten the authored text before retrying")
...
def _run_tts_engine(engine, text, output_wav, rate="+0%", pitch="+0Hz", emotion=None):
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if engine == "doubao-tts":
                from aihub_adapter import synthesize
                synthesize(text, output_wav, rate=rate)
...
def resolve_tts_engine():
    ...
    if engine == "doubao-tts":
        ...
        credentials()
        if CONFIG["voice_ref"]:
            raise RuntimeError("Doubao adapter supports preset voices only")
        return engine
...
def tts_settings_fingerprint(engine):
    ...
    if engine == "doubao-tts":
        from aihub_adapter import tts_settings
        settings.update(tts_settings())
    voice_ref = CONFIG["voice_ref"]
...
def main():
    ...
    ap.add_argument(
        ...
        choices=["auto", "mimo-tts", "fish-audio", "doubao-tts"],
