...
def _continuation_command(video, work_dir, args):
    ...
    if getattr(args, "film_title", None):
        parts += ["--film-title", args.film_title]
    if getattr(args, "film_year", None) is not None:
        parts += ["--film-year", str(args.film_year)]
    if not getattr(args, "research_film", True):
        parts.append("--no-research-film")
...
def _brief_excerpt(path, limit=1200):
    ...
    preferred = [
        value
        for value in (
            section("## Selected opening brief (from opening_brief.json)"),
            section("## Film research (from film_research.json)"),
