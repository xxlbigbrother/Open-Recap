...
def detect_scenes(video_path, work_dir, threshold=None):
    ...
    # Full-film analysis: bound sparse sampling windows without imposing edit lengths.
    max_seconds = float(os.environ.get("ANALYSIS_MAX_SCENE_SECONDS", "0"))
    if max_seconds > 0:
        import math
        expanded = []
        for scene in scenes:
            count = max(1, math.ceil((scene["end"] - scene["start"]) / max_seconds))
            for i in range(count):
                expanded.append({"start": round(scene["start"] + (scene["end"]-scene["start"])*i/count, 3),
                                 "end": round(scene["start"] + (scene["end"]-scene["start"])*(i+1)/count, 3)})
        scenes = expanded
    # 保存
