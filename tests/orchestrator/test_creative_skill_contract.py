...
STAGE_SKILL_NAMES = tuple(
    path.name
    for path in sorted(SKILLS_ROOT.iterdir())
    if path.is_dir() and (path / "SKILL.md").is_file()
)
SKILL_NAMES = ("openrecap", *STAGE_SKILL_NAMES)


def _skill_path(skill_name: str) -> Path:
    return SKILLS_ROOT / "SKILL.md" if skill_name == "openrecap" else SKILLS_ROOT / skill_name / "SKILL.md"
...
def test_markdown_references_resolve_within_stage_or_shared_references():
    for skill_name in STAGE_SKILL_NAMES:
        ...
        for markdown_path in skill_dir.rglob("*.md"):
            ...
            for reference in references:
                ...
                try:
                    allowed_roots = (skill_dir.resolve(), (SKILLS_ROOT / "references").resolve())
                    if not any(resolved.is_relative_to(root) for root in allowed_roots):
                        raise ValueError
                except ValueError:
                    pytest.fail(f"{markdown_path}: reference escapes stage and shared references: {reference}")
...
@pytest.mark.parametrize("skill_name", STAGE_SKILL_NAMES)
