...
def _analysis_settings(args):
    # Film research deliberately stays out of this fingerprint. It runs after
    # understanding and must not invalidate ASR/VLM artifacts.
    return {
...
def _run_film_research_if_requested(work_dir, args):
    """Run source-backed film research after understanding, before authoring."""
    title = str(getattr(args, "film_title", "") or "").strip()
    if not title or not bool(getattr(args, "research_film", True)):
        return False
    command = ["--title", title, "--work-dir", str(work_dir)]
    year = getattr(args, "film_year", None)
    if year is not None:
        command += ["--year", str(year)]
    _run("video-script", "research_film.py", *command)
    return True
