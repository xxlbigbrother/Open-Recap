...
def assembly_settings_fingerprint(work_dir=None):
    ...
    fingerprint = {
        ...
        "video_filters": {
            ...
            "subtitle_mask_bridge_seconds": (
                CONFIG.get("subtitle_mask_bridge_seconds", 0.001) if mask_source_subtitles else None
            ),
            "subtitle_y_top": CONFIG["subtitle_y_top"],
            "subtitle_y_bot": CONFIG["subtitle_y_bot"],
            "source_subtitle_mask_windows_fingerprint": (
                _artifact_fingerprint(Path(work_dir)/'source_subtitle_mask_windows.json')
                if work_dir is not None and mask_source_subtitles else None
            ),
            "visual_overlays": {
