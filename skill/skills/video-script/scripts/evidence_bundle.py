...
def _research_context_items(research):
    ...
    if isinstance(film_research, dict):
        ...
        if isinstance(film, dict):
            for key in (
                "title", "description", "director", "screenwriter", "cast",
                "composer", "production_company", "action_design", "genre",
                "release_dates", "awards",
            ):
                value = film.get(key)
                if isinstance(value, list):
                    value = "、".join(_clip_text(item, 100) for item in value[:10])
                value = _clip_text(value, 500)
                if value:
                    items.append({
                        "id": f"research:film:{key}",
                        "source": "research",
                        "clock": None,
                        "source_id": "film_research",
                        "start": None,
                        "end": None,
                        "text": f"{key}: {value}",
                        "support": "context_only",
    ...
    if isinstance(opening_brief, dict):
        ...
        for key, text in (("intro", intro), ("promise", promise)):
            if text:
                items.append({
                    "id": f"research:opening:{key}",
                    "source": "research",
                    "clock": None,
                    "source_id": "opening_brief",
                    "start": None,
                    "end": None,
                    "text": text,
                    "support": "context_only",
