...
def test_review_reads_source_backed_film_research(tmp_path):
    (tmp_path / "film_research.json").write_text(
        json.dumps(
            {
                "film": {
                    "title": "功夫",
                    "director": ["周星驰"],
                    "genre": ["动作喜剧"],
                    "release_dates": ["2004-12-23"],
                },
                "sources": [
                    {"id": "web-1", "url": "https://example.com/kungfu"}
                ],
            },
            ensure_ascii=False,
        ),
    ...
    research = review_response._load_review_research_context(tmp_path)
    text = review_response._format_review_research_context(research)
    assert "Film research" in text
    assert "周星驰" in text
    assert "web-1" in text

    bundle = review_response.build_evidence_bundle(
        [], [], [{"start": 0, "end": 1, "narration": "测试"}], research=research
    )
    assert any(item["id"] == "research:film:director" for item in bundle["context_items"])


def test_review_reads_selected_opening_brief(tmp_path):
    (tmp_path / "opening_brief.json").write_text(
        json.dumps(
            {
                "one_sentence_intro": "2004年的《功夫》由周星驰执导。",
                "opening_promise": "看懂电影怎样把高手藏进普通生活。",
            },
            ensure_ascii=False,
        ),
        encoding="utf-8",
    ...
    research = review_response._load_review_research_context(tmp_path)
    text = review_response._format_review_research_context(research)
    assert "Selected opening brief" in text
    assert "把高手藏进普通生活" in text
    bundle = review_response.build_evidence_bundle(
        [], [], [{"start": 0, "end": 1, "narration": "测试"}], research=research
    )
    assert any(item["id"] == "research:opening:promise" for item in bundle["context_items"])
