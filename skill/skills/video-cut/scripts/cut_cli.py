...
def main():
    ...
    original_map = work_dir / "original_source_map.json"
    if original_map.exists():
        source_map = json.loads(original_map.read_text())
        geometry_qc.update(width=source_map["delivery_width"], height=source_map["delivery_height"],
                           fps=source_map["delivery_fps"], reason="original_source_delivery")
        validated_plan["original_source_map"] = source_map
