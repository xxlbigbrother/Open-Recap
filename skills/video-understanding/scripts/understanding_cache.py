...
def _scene_cache_payload(video_path):
    return {
        ...
        "settings": {
            ...
            "analysis_max_scene_seconds": __import__("os").environ.get("ANALYSIS_MAX_SCENE_SECONDS"),
...
def _asr_cache_payload(video_path, *, skip_asr=False):
    from aihub_adapter import asr_settings
    return {
        "adapter": asr_settings(),
        "schema_version": 1,
        "stage": "asr",
        "source_video_fingerprint": file_fingerprint(video_path),
        "settings": {
...
def _vlm_cache_payload(video_path, work_dir, scenes_json, frames):
    return {
        "schema_version": 1,
        "stage": "vlm",
        "asr_context_fingerprint": _artifact_fingerprint(Path(work_dir) / "asr_result.json"),
        "source_video_fingerprint": file_fingerprint(video_path),
