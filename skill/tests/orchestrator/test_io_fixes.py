...
def test_continuation_command_preserves_film_research_flags():
    args = _manifest_args(film_title="功夫", film_year=2004)
    command = recap_timeline._continuation_command("/tmp/in.mp4", "/tmp/work", args)
    assert "--film-title" in command and "功夫" in command
    assert "--film-year 2004" in command

    disabled = _manifest_args(film_title="功夫", research_film=False)
    command = recap_timeline._continuation_command("/tmp/in.mp4", "/tmp/work", disabled)
    assert "--no-research-film" in command
