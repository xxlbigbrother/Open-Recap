...
def config(stage,provider=None):
    ...
    env['PYTHONPATH']=str(ROOT/'skill/skills'/stage/'scripts')+os.pathsep+str(ROOT)
...
def test_explicit_missing_env_path_fails_before_running_stage(tmp_path):
    ...
    p=subprocess.run([sys.executable,str(ROOT/'run_skill.py'),'video-understanding','understand.py','--help'],
