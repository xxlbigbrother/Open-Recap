...
def test_short_mask_gaps_bridge_without_covering_long_original_scene(monkeypatch, tmp_path):
    """A brief narration breath must not flash the burned source caption back on."""
    monkeypatch.setitem(CONFIG, "burn_subtitles", True)
    monkeypatch.setitem(CONFIG, "mask_source_subtitles", True)
    monkeypatch.setitem(CONFIG, "source_subtitle_mask_policy", "opt_in")
    monkeypatch.setitem(CONFIG, "source_subtitle_mask_timing", "narration")
    ...
    monkeypatch.setitem(CONFIG, "subtitle_y_top", 610)
    monkeypatch.setitem(CONFIG, "subtitle_y_bot", 700)
    segments = [{"actual_place_start": a, "actual_place_end": b, "narration": "旁白"}
                for a, b in [(0.2, 2.0), (2.5, 4.0), (8.0, 9.0)]]
    monkeypatch.setitem(CONFIG, "subtitle_mask_bridge_seconds", 0.001)
    initial = _source_subtitle_mask_filter(_canvas(), tmp_path, segments, 10)
    first_fingerprint = assembly_settings_fingerprint(tmp_path)
    assert initial.count("drawbox=") == 3
    monkeypatch.setitem(CONFIG, "subtitle_mask_bridge_seconds", 0.8)
    bridged = _source_subtitle_mask_filter(_canvas(), tmp_path, segments, 10)
    assert bridged.count("drawbox=") == 2
    assert "between(t,0.200,4.000)" in bridged
    assert "between(t,8.000,9.000)" in bridged
    assert assembly_settings_fingerprint(tmp_path) != first_fingerprint


def test_muted_source_caption_window_stays_masked_after_narration_ends(monkeypatch, tmp_path):
    import json
    monkeypatch.setitem(CONFIG, 'burn_subtitles', True)
    monkeypatch.setitem(CONFIG, 'mask_source_subtitles', True)
    monkeypatch.setitem(CONFIG, 'source_subtitle_mask_policy', 'opt_in')
    ...
    monkeypatch.setitem(CONFIG, 'subtitle_original_in_gaps', False)
    monkeypatch.setitem(CONFIG, 'subtitle_y_top', 610)
    monkeypatch.setitem(CONFIG, 'subtitle_y_bot', 710)
    monkeypatch.setitem(CONFIG, 'subtitle_mask_bridge_seconds', .001)
    segments=[{'actual_place_start':1,'actual_place_end':3,'narration':'旁白'}]
    (tmp_path/'source_subtitle_mask_windows.json').write_text(json.dumps({'schema_version':1,'clock':'output','windows':[{'start':0,'end':5,'reason':'source_muted'}]}))
    filt=_source_subtitle_mask_filter(_canvas(),tmp_path,segments,10)
    assert 'between(t,0.000,5.000)' in filt
    assert filt.count('drawbox=')==1
    before=assembly_settings_fingerprint(tmp_path)
    (tmp_path/'source_subtitle_mask_windows.json').write_text(json.dumps({'schema_version':1,'clock':'output','windows':[{'start':0,'end':6,'reason':'source_muted'}]}))
    assert assembly_settings_fingerprint(tmp_path)!=before


def test_invalid_source_caption_mask_windows_are_not_silently_applied(monkeypatch,tmp_path):
    import json
    monkeypatch.setitem(CONFIG,'burn_subtitles',True)
    monkeypatch.setitem(CONFIG,'mask_source_subtitles',True)
    monkeypatch.setitem(CONFIG,'source_subtitle_mask_policy','opt_in')
    monkeypatch.setitem(CONFIG,'source_subtitle_mask_timing','narration')
    monkeypatch.setitem(CONFIG,'subtitle_mask_opacity',1.0)
    (tmp_path/'source_subtitle_mask_windows.json').write_text(json.dumps({'schema_version':1,'clock':'output','windows':[{'start':0,'end':float('nan')}]}))
    with pytest.raises(ValueError,match='mask window'):_source_subtitle_mask_filter(_canvas(),tmp_path,[],10)


def test_source_handoff_returns_in_measured_gap_before_unrelated_next_sentence(monkeypatch, tmp_path):
    """One earlier grunt must not duck the next scene's first line."""
    payload={'require_measured':True,'sentence_anchors':[{'time':12,'pause_start':11.8,'confidence':'high'}],
             'speech_spans':[{'start':3,'end':3.3},{'start':10,'end':11.7}],
             'quiet_windows':[{'start':5,'end':7}]}
    (tmp_path/'speech_boundary_anchors.json').write_text(json.dumps(payload))
    monkeypatch.setitem(CONFIG,'duck_fade_seconds',.2)
    segment={'actual_place_start':1,'actual_place_end':5.5,'overlaps_speech':True}
    report=audio_mix._apply_source_sentence_handoffs([segment],tmp_path,15)
    assert segment['source_duck_end']==5.5
    assert segment['source_restore_at']==pytest.approx(5.7)
    assert report[0]['status']=='measured_quiet_release'


@pytest.mark.parametrize('quiet',[[{'start':5,'end':5.6}],[{'start':5,'end':6.5}]])
def test_source_handoff_never_uses_too_short_or_speech_contaminated_gap(monkeypatch,tmp_path,quiet):
    payload={'require_measured':True,'sentence_anchors':[{'time':12,'pause_start':11.8,'confidence':'high'}],
             'speech_spans':[{'start':3,'end':3.3},{'start':5.6,'end':6.4},{'start':10,'end':11.7}],
             'quiet_windows':quiet}
    (tmp_path/'speech_boundary_anchors.json').write_text(json.dumps(payload))
    monkeypatch.setitem(CONFIG,'duck_fade_seconds',.2)
    segment={'actual_place_start':1,'actual_place_end':5.5,'overlaps_speech':True}
    audio_mix._apply_source_sentence_handoffs([segment],tmp_path,15)
    assert segment['source_restore_at']==12


def test_quiet_exit_does_not_authorize_unanchored_entry_inside_source_speech(monkeypatch,tmp_path):
    payload={'require_measured':True,'sentence_anchors':[],
             'speech_spans':[{'start':1,'end':3.3},{'start':10,'end':11.7}],
             'quiet_windows':[{'start':5,'end':7}]}
    (tmp_path/'speech_boundary_anchors.json').write_text(json.dumps(payload))
    monkeypatch.setitem(CONFIG,'duck_fade_seconds',.2)
    segment={'actual_place_start':2,'actual_place_end':5.5,'overlaps_speech':True}
    audio_mix._apply_source_sentence_handoffs([segment],tmp_path,15)
    assert segment['source_handoff_blocking'] is True
    assert segment['source_entry_status']=='anchors_unavailable'
    assert segment['source_restore_at']==pytest.approx(5.7)
