...
需要边看边解读的回放或定格时，先阅读 [显式呈现计划](references/presentation-plan.md)，使用可选 `scripts/presentation.py`。它输出 `presentation_source.mp4` 与逐操作源/输出映射，不改变普通 `cut.py` 流程。计划要以实际配音时长验证证据窗口与原声保护，不能把无法执行的 `freeze` 字段塞进普通剪辑列表。
