...
def _apply_source_sentence_handoffs(tts_segments, work_dir, video_duration):
    ...
    for run in runs:
        ...
        quiet_release = next((row for row in quiet_windows
            if row['start'] <= run['end'] and row['end'] >= run['end'] + fade
            and not any(s['start'] < run['end'] + fade and s['end'] > run['end']
                        for s in speech_spans)), None)
        if (quiet_release is not None and
                (restore_anchor is None or run['end'] + fade < restore_anchor['time'])):
            # Narration may cover an earlier grunt yet end well before the next scene.
            # A measured quiet window at its tail avoids suppressing that next line.
            duck_end = run['end']
            restore_at = run['end'] + fade
            status = 'measured_quiet_release'
