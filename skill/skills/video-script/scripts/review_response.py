...
def _load_review_research_context(work_dir):
    combined = {}
    for filename in ("background_research.json", "film_research.json", "opening_brief.json"):
        ...
        if not isinstance(data, dict):
            continue
        if filename == "background_research.json":
            combined.update(data)
        elif filename == "film_research.json":
            combined["film_research"] = data
        else:
            combined["opening_brief"] = data
    return combined
...
def _format_review_research_context(research, limit=1200):
    ...
    film_research = research.get("film_research")
    if isinstance(film_research, dict):
        film = film_research.get("film")
        if isinstance(film, dict):
            lines.append("- Film research:")
            for key, label in (
                ("title", "Title"),
                ...
                if isinstance(value, list):
                    value = "、".join(_clip_text(item, 80) for item in value[:8])
                value = _clip_text(value, 400)
                if value:
                    lines.append(f"    - {label}: {value}")
        sources = film_research.get("sources")
        if isinstance(sources, list) and sources:
            lines.append("    - Source IDs: " + ", ".join(
                _clip_text(source.get("id"), 50)
                for source in sources[:10]
                if isinstance(source, dict) and source.get("id")
            ))

    opening_brief = research.get("opening_brief")
    if isinstance(opening_brief, dict):
        intro = _clip_text(opening_brief.get("one_sentence_intro"), 500)
        promise = _clip_text(opening_brief.get("opening_promise"), 500)
        if intro or promise:
            lines.append("- Selected opening brief:")
        if intro:
...
def build_grounding_qc(work_dir, review, bundle, *, timeline="source"):
    ...
    return {
        ...
        "source_fingerprints": {
            ...
            "film_research": _source_fingerprint(work_dir, "film_research.json"),
        ...
        "index_inputs": {
            ...
            "asr": bool(asr_items),
            "research": bool(
                _source_fingerprint(work_dir, "background_research.json")
                or _source_fingerprint(work_dir, "film_research.json")
            ),
        },
