...
电影片名明确时传 `--film-title`（可选 `--film-year`）。编排器在视频理解完成后、写稿之前执行影片资料检索并生成 `film_research.json`。这一步只补主创、创作背景和可引用知识，不改变或重跑理解结果；Agent 随后写 `opening_brief.json`，从中选择少量真正服务本期角度的信息。若无需搜索可传 `--no-research-film`。

```bash
python3 scripts/recap.py movie.mp4 --work-dir work \
  --film-title "功夫" --film-year 2004 --edit-mode cut
```
