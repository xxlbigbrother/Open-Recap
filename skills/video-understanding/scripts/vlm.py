...
def analyze_scenes(scenes, frames, work_dir, *, resume=True):
    ...
    def _analyze_single_scene(i, scene):
        ...
        asr_path = Path(work_dir) / "asr_result.json"
        if asr_path.exists():
            asr_rows = json.loads(asr_path.read_text())
            dialogue = [f"{x['start']:.1f}-{x['end']:.1f}: {x['text']}" for x in asr_rows
                        if x['end'] > scene['start'] and x['start'] < scene['end']]
            frame_ts_text += "\n本段原声转写（可能有错字，人物动作以画面为准）：\n" + "\n".join(dialogue)[:3500]
    ...
    def _scene_cache_key(i, scene):
        ...
        if 'understanding_provider_settings' in CONFIG:
            key += '|' + stable_hash(CONFIG['understanding_provider_settings'])
        return key
...
def mimo_video_settings_fingerprint():
    ...
    return {
        **({'provider_settings': CONFIG['understanding_provider_settings']} if 'understanding_provider_settings' in CONFIG else {}),
