...
def _load_film_research(work_dir):
    """Load source-backed film metadata produced after video understanding."""
    path = Path(work_dir) / "film_research.json"
    if not path.exists():
        return {}
    try:
...
def _load_opening_brief(work_dir):
    path = Path(work_dir) / "opening_brief.json"
    if not path.exists():
        return {}
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
...
def _format_film_research(research, limit=2200):
    """Render source-backed film research for opening and scene-local knowledge."""
    if not research:
        return []
    film = research.get("film") if isinstance(research.get("film"), dict) else {}
    sources = research.get("sources") if isinstance(research.get("sources"), list) else []
    candidates = research.get("opening_candidates") if isinstance(research.get("opening_candidates"), list) else []
    lines = [
        "## Film research (from film_research.json)",
        "",
        "Use this after video understanding and before writing. It is source-backed context, not current-frame evidence. Select only facts that serve the current commentary angle; do not read search extracts verbatim or turn the opening into a credits list.",
        "",
    ]
    title = _clip_text(film.get("title"), 120)
    description = _clip_text(film.get("description"), 300)
    if title:
    ...
    for key, label in (
        ...
        if isinstance(values, list) and values:
            lines.append(f"- {label}: " + "、".join(_clip_text(x, 80) for x in values[:10]))
    if candidates:
        lines.extend(["", "- Opening candidates (choose 2–4, then write `opening_brief.json`):"])
        for candidate in candidates[:8]:
            if not isinstance(candidate, dict):
                continue
            claim = _clip_text(candidate.get("claim"), 500)
            why = _clip_text(candidate.get("why_useful"), 180)
            source_ids = ",".join(str(x) for x in candidate.get("source_ids", [])[:4])
            if claim:
                lines.append(f"    - [{candidate.get('kind', 'fact')}; sources={source_ids or 'missing'}] {claim}")
                if why:
                    lines.append(f"      Why it may help: {why}")
    if sources:
        ...
        for source in sources[:12]:
            if not isinstance(source, dict):
                continue
            lines.append(
                f"    - {source.get('id', 'source')}: {_clip_text(source.get('title'), 100)} | "
                f"{_clip_text(source.get('url'), 240)}"
            )
    lines.extend([
        "",
        "Before narration, write `opening_brief.json` with: one_sentence_intro; selected_facts (claim, purpose, source_ids); opening_promise; deferred_facts. The intro normally needs title/year/director or another compact identity plus one angle-relevant background fact. Awards, box office and cast lists are optional, not a completeness checklist.",
        "",
    ])
    text = "\n".join(lines)
    if len(text) <= limit:
        return lines
    clipped = text[:limit].rsplit("\n", 1)[0].rstrip()
    return [*clipped.splitlines(), "", "[Film research clipped; read film_research.json for all sources]", ""]


def _format_opening_brief(brief, limit=1400):
    if not brief:
        return []
    lines = [
        "## Selected opening brief (from opening_brief.json)",
        "",
        "Treat this as the accepted opening research selection. Preserve its factual claims and source IDs while rewriting for spoken flow; revise it only when the commentary angle changes or a source is rejected.",
        "",
    ]
    intro = _clip_text(brief.get("one_sentence_intro"), 500)
    promise = _clip_text(brief.get("opening_promise"), 500)
    if intro:
        lines.append(f"- One-sentence intro: {intro}")
    if promise:
        lines.append(f"- Opening promise: {promise}")
    facts = brief.get("selected_facts")
    if isinstance(facts, list) and facts:
        lines.append("- Selected facts:")
        for item in facts[:6]:
            if not isinstance(item, dict):
                continue
            claim = _clip_text(item.get("claim"), 400)
            purpose = _clip_text(item.get("purpose"), 180)
            source_ids = ",".join(str(x) for x in item.get("source_ids", [])[:4])
            if claim:
                lines.append(f"    - [{source_ids or 'missing source'}] {claim}")
                if purpose:
                    lines.append(f"      Purpose: {purpose}")
    deferred = brief.get("deferred_facts")
    if isinstance(deferred, list) and deferred:
        lines.append("- Deferred facts: " + "；".join(_clip_text(x, 180) for x in deferred[:8]))
    text = "\n".join(lines)
    if len(text) <= limit:
        return [*lines, ""]
    return [*text[:limit].rsplit("\n", 1)[0].splitlines(), "", "[Opening brief clipped]", ""]
...
def _load_consolidation(work_dir, scenes_analysis):
    ...
    if not meta.items() >= expected.items():
        return {}
    if meta.get('provider_settings') != CONFIG.get('understanding_provider_settings'):
        return {}
    try:
